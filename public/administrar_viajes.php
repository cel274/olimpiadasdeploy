...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accionReserva'])) {
        ...
        if ($_POST['accion'] === 'eliminar') {
            $id = $_POST['id'];
            $stmt = $conn->prepare("SELECT destino FROM viajes WHERE id = ?");
            ...
            $pais = $_POST['pais'];
            $precio = $_POST['precio'];
            $stmt = $conn->prepare("UPDATE viajes SET destino = ?, pais = ?, precio = ? WHERE id = ?");
            ...
            if ($imagen && $imagen['error'] === 0) {
                ...
                if ($tipo === 'image/jpeg') {
                    ...
                    imagejpeg($origen, $rutaDestino, 90);
                    imagedestroy($origen);
                    $stmt = $conn->prepare("INSERT INTO viajes (destino, pais, precio) VALUES (?, ?, ?)");
...
$viajes = $conn->query("SELECT * FROM viajes ORDER BY id DESC");
...
function enviarFormularioCambio(id) {
    document.getElementById('form-cambiar-img-' + id).submit();
}
</script>
</head>
<body>
